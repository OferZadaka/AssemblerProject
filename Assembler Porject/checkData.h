...
typedef enum {
	NUMBER = 1, /*0001 in binary*/
	LABEL = 2, /*0010 in binary*/
	HASH = 4, /*0100 in binary*/
	REGISTER = 8, /*1000 in binary*/
	ERROR = -1
} OperandType;
...
typedef struct
{
	...
	int address_field;			/* used for Struct type. The address of the field in the memory */
} operandInfo;
...
/* Returns if string con tains only white spaces. */
...
/*Checks if the register is a hash register*/
bool checkHashRegister(char *str, int *value);
